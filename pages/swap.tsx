...
import { sendAnalyticsEvent } from 'swap/analytics'
import { ElementName, Event, EventName, PageName, SectionName } from 'swap/analytics/constants'
import { Trace } from 'swap/analytics/Trace'
import { TraceEvent } from 'swap/analytics/TraceEvent'
import {
  formatPercentInBasisPointsNumber,
  formatToDecimal,
  getDurationFromDateMilliseconds,
  getTokenAddress,
} from 'swap/analytics/utils'
import { sendEvent } from 'swap/components/analytics'
import { NetworkAlert } from 'swap/components/NetworkAlert/NetworkAlert'
import PriceImpactWarning from 'swap/components/swap/PriceImpactWarning'
import SwapDetailsDropdown from 'swap/components/swap/SwapDetailsDropdown'
import UnsupportedCurrencyFooter from 'swap/components/swap/UnsupportedCurrencyFooter'
import TokenSafetyModal from 'swap/components/TokenSafety/TokenSafetyModal'
import { MouseoverTooltip } from 'swap/components/Tooltip'
import { isSupportedChain } from 'swap/constants/chains'
import { NavBarVariant, useNavBarFlag } from 'swap/featureFlags/flags/navBar'
import { RedesignVariant, useRedesignFlag } from 'swap/featureFlags/flags/redesign'
import { useSwapCallback } from 'swap/hooks/useSwapCallback'
...
import { useToggleWalletModal } from 'swap/state/application/hooks'
import { InterfaceTrade } from 'swap/state/routing/types'
import { TradeState } from 'swap/state/routing/types'
import styled, { css, useTheme } from 'styled-components/macro'

import AddressInputPanel from 'swap/components/AddressInputPanel'

import { ButtonConfirmed, ButtonError, ButtonLight, ButtonPrimary } from 'swap/components/Button'
import { GreyCard } from 'swap/components/Card'
import { AutoColumn } from 'swap/components/Column'
import SwapCurrencyInputPanel from 'swap/components/CurrencyInputPanel/SwapCurrencyInputPanel'
import Loader from 'swap/components/Loader'
import { AutoRow } from 'swap/components/Row'
import confirmPriceImpactWithoutFee from 'swap/components/swap/confirmPriceImpactWithoutFee'
import ConfirmSwapModal from 'swap/components/swap/ConfirmSwapModal'
import {
  ArrowWrapper,
  PageWrapper,
  SwapCallbackError,
  SwapWrapper,
} from 'swap/components/swap/styleds'
import SwapHeader from 'swap/components/swap/SwapHeader'
import { SwitchLocaleLink } from 'swap/components/SwitchLocaleLink'
import TokenWarningModal from 'swap/components/TokenWarningModal'
import { TOKEN_SHORTHANDS } from 'swap/constants/tokens'
import { useAllTokens, useCurrency } from 'swap/hooks/Tokens'
import { ApprovalState, useApproveCallbackFromTrade } from 'swap/hooks/useApproveCallback'
import useENSAddress from 'swap/hooks/useENSAddress'
import { useERC20PermitFromTrade, UseERC20PermitState } from 'swap/hooks/useERC20Permit'
import useIsArgentWallet from 'swap/hooks/useIsArgentWallet'
import { useIsSwapUnsupported } from 'swap/hooks/useIsSwapUnsupported'
import { useStablecoinValue } from 'swap/hooks/useStablecoinPrice'
import useWrapCallback, { WrapErrorText, WrapType } from 'swap/hooks/useWrapCallback'
import { Field } from 'swap/state/swap/actions'
import {
  useDefaultsFromURLSearch,
  useDerivedSwapInfo,
  useSwapActionHandlers,
  useSwapState,
} from 'swap/state/swap/hooks'
import { useExpertModeManager } from 'swap/state/user/hooks'
import { LinkStyledButton, ThemedText } from 'swap/theme'
import { computeFiatValuePriceImpact } from 'swap/utils/computeFiatValuePriceImpact'
import { maxAmountSpend } from 'swap/utils/maxAmountSpend'
import { computeRealizedPriceImpact, warningSeverity } from 'swap/utils/prices'
import { supportedChainId } from 'swap/utils/supportedChainId'
                            ...
                            <AutoRow justify="space-between" style={{ flexWrap: 'nowrap' }}>
                              ...
                              {approvalState === ApprovalState.PENDING ? (
                                ...
                                <MouseoverTooltip
                                  text={
                                    <Trans>
                                      You must give the Uniswap smart contracts permission to use
                                      your {currencies[Field.INPUT]?.symbol}. You only have to do
                                      this once per token.
                                    </Trans>
                                  }
                                >
